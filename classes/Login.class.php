...
class Login extends Page
{
  ...
  private function handlePOST( $posted )
  {
    ...
    if( $success === True )
    {
      ...
      if( $_SESSION['userRole'] == '1' )
      {
        $this->content .= '
        <script>
          window.location = "index.php?display=ApproverMenu";
        </script>
        ';
      }
      if( $_SESSION['userRole'] == '2' )
      {
        $this->content .= '
        <script>
          window.location = "index.php?display=POCMenu";
        </script>
        ';
      }
      ...
      if( $_SESSION['userRole'] == '4' )
      {
        $this->content .= '
        <script>
          window.location = "index.php?display=GlobalAdminMenu";
        </script>
        ';
      }
    }
  ...
  private function login( $posted )
  {
    ...
    if( $check[0]['Count'] == 1 )
    {
      $data = $this->db->query_DB("SELECT U.Username, U.Role, M.Org_ID
                                   FROM Users U, Membership M
                                   WHERE U.Username = '" . $posted['username'] . "'
